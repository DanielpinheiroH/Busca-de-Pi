...
ARQUIVO_PLANILHA = BASE_DIR / "backend" / "data" / "base.xlsb"
...
def main():
    if not ARQUIVO_PLANILHA.exists():
        raise FileNotFoundError(
            f"Planilha não encontrada: {ARQUIVO_PLANILHA}"
        )
    ...
    with open_workbook(str(ARQUIVO_PLANILHA)) as wb:
        ...
        if ABA_DADOS not in wb.sheets:
            raise ValueError(
                f"A aba '{ABA_DADOS}' não foi encontrada."
            )
    ...
    if idx_cabecalho is None:
        ...
        for i, row in enumerate(rows[:10], start=1):
            print(
                f"Linha {i}: "
                f"{[limpar_cabecalho(c) for c in row]}"
            )
    ...
    for linha in rows[idx_cabecalho + 1:]:
        ...
        item = {
            ...
            "anunciante": normalizar_texto(
                registro.get("Nome do Anunciante")
            ),
            "cnpjAnunciante": normalizar_texto(
                registro.get("CNPJ do Anunciante")
            ),
            "tipoPi": normalizar_texto(
                registro.get("Sub Perfil Anunciante")
            ),
            "piMatriz": normalizar_texto(
                registro.get("PI Matriz")
            ),
            "campanha": normalizar_texto(
                registro.get("Nome Campanha")
            ),
            "executivo": normalizar_texto(
                registro.get("Executivo")
            ),
            "diretoria": normalizar_texto(
                registro.get("Diretoria")
            ),
            ...
            "agencia": normalizar_texto(
                registro.get("Nome da Agência")
            ),
            "razaoSocialAgencia": normalizar_texto(
                registro.get("Razão Social Agência")
            ),
            "cnpjAgencia": normalizar_texto(
                registro.get("CNPJ Agência")
            ),
            "ufCliente": normalizar_texto(
                registro.get("UF Cliente")
            ),
            "ufAgencia": normalizar_texto(
                registro.get("UF Agência")
            ),
            "perfil": normalizar_texto(
                registro.get("Perfil Anunciante")
            ),
            "mesVenda": normalizar_mes(
                registro.get("Mes da venda")
            ),
            "dataVenda": normalizar_data(
                registro.get("Data da venda")
            ),
            ...
            "fimVeiculacao": normalizar_data(
                registro.get("Data Final Veiculação")
            ),
            "vencimento": normalizar_data(
                registro.get("Vencimento")
            ),
            "valorBruto": normalizar_numero(
                registro.get("Valor bruto")
            ),
            "valorLiquido": normalizar_numero(
                registro.get("Valor líquido")
            ),
